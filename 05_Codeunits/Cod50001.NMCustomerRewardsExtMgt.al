codeunit 50001 NMCustomerRewardsExtMgt
{
    ...
    var
        ActivationCodeInformation: Record NMActivationCodeInformation;
    begin
    ...
    var
        CustomerRewardsWizard: Page NMCustomerRewardsWizard;
    begin
    ...
    var
        RewardsLevelList: Page NMRewardsLevelList;
    begin
    ...
    [EventSubscriber(ObjectType::Codeunit, Codeunit::NMCustomerRewardsExtMgt, 'OnGetActivationCodeStatusFromServer', '', false, false)]
    local procedure OnGetActivationCodeStatusFromServerSubscriber(ActivationCode: Text);
    var
        ActivationCodeInfo: Record NMActivationCodeInformation;
        ResponseText: Text;
        Result: JsonToken;
    ...
    begin
        if CustomerRewardsMgtSetup.Get() then
            exit(CustomerRewardsMgtSetup."Cust. Rew. Ext. Mgt. Cod. ID" = CODEUNIT::NMCustomerRewardsExtMgt);
        exit(false);
    end;
